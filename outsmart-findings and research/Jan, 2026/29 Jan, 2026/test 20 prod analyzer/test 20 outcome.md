...
DOPO UN PÓ DI RICERCA...
Sono tornato dopo aver svolto delle ricerche online, e mi risulta che non sia possibile in alcun modo accedere all'interno dei subnode e quindi non posso avere espressioni che si risolvono sulla base del valore di certe chiavi presenti all'interno di essi, questo significa che per ovviare a questo problema, alla fine, l'unica cosa che ci rimane possibile fare, é intrapprendere un nuovo capitolo in questo sviluppo del sistema Outsmart. Questo nuovo capitolo riguarda la costruzione di uno ""spazio di lavoro globale"", cioé un oggetto di memoria per immagazzinare accuratamente tutto ció che riguarda l'utente Boss ed il suo prodotto o servizio, con al di sotto di questo oggetto di memoria per singolo utente del mio sistema, i suoi lead e le loro informazioni, con all'interno di questi oggetti, i loro oggetti di memoria personali, con all'interno, ció che scoperto ed aggiornato grazie alla ricerca forense, e per finire le varie interazioni che avvengono tra la facciata dell'utente (Tatemae) e il singolo lead.

**MOMENTANEAMENTE** terrei l'architettura attuale riguardo la memoria semplice dell'analyzer, ma andrei a switchare alla memoria vera e propria, condivisa, nel momento in cui passiamo alla produzione.
